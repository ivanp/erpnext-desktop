...
public sealed class ManagedRuntimeResolverTests : IDisposable
{
    ...
    [Fact]
    public void GenerateCertificates_WritesUsableMutualTlsMaterial()
    {
        var certificateDirectory = Path.Combine(_tempRoot, "certificates");
        var store = new TlsCertificateStore(certificateDirectory);

        store.GenerateCertificates();

        Assert.True(store.LoadClientCert().HasPrivateKey);
        Assert.True(File.Exists(Path.Combine(store.QemuCertDir, "server-key.pem")));
    }
    // ── Helpers ───────────────────────────────────────────────────────────
